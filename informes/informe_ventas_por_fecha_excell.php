...
$sql_traer_items  = "
select f.numero_factura,o.orden,o.fecha,io.* 
from item_orden io 
inner join ordenes o on (o.id = io.no_factura) 
inner join facturas f on (f.id_orden = o.id)
where 1=1 ";


if($_REQUEST['fechain'] =='' &&  $_REQUEST['fechafin'] ==''){
	$sql_traer_items .= " and f.fecha >= '".$fechahoy."' ";
	$sql_traer_items .= " and f.fecha <= '".$fechahoy."' ";
}

if($_REQUEST['fechain'] !=''){
	$sql_traer_items .= " and f.fecha >= '".$_REQUEST['fechain']."' ";
}
if($_REQUEST['fechafin'] !=''){
	$sql_traer_items .= " and f.fecha <= '".$_REQUEST['fechafin']."' ";
}
...
while($items = mysql_fetch_assoc($consulta_items))
{
	...
	echo '<td>'.$items['numero_factura'].'</td>';
