...
////////////////////////////////////////////////////////////////////
function muestre_items_nuevo_pos($orden,$tabla,$conexion,$id_empresa,$resolucion)
		{
				$subtotal = 0;
				$sql_items_orden = "select * from  $tabla where no_factura = '".$orden."' and id_empresa = '".$id_empresa."'  order by id_item ";
				$consulta_items = mysql_query($sql_items_orden,$conexion);
			 ...
			 $i++;
	 			echo '<tr>
				 <td width="70%" >'.strtoupper($items['descripcion']).'</td>
                <td width="10%"><div align="center">'.$items['cantidad'].'</div></td>
				<td width="20%"><div align = "right"> '.'$'.number_format($items['total_item'], 0, ',', '.').'</div></td>';
			    $total_factura +=   $items['total_item'];
			}
			echo '<tr>';
			echo '<tr>';
			echo '<td align = "right">Total</td>';
			echo '<td></td>';
			echo '<td ><div align = "right"> '.'$'.number_format($total_factura, 0, ',', '.').'</div></td>';
			echo '</tr>';
			$filas = 20-$i; 
			for($j=1 ; $j<=$filas ; $j++){
